...
def match_progression(match):
    ...
    return fig_wickets
nav=option_menu(menu_title=None,options=["win percentage calculator","match analysis","Score predictor"],orientation="horizontal")
...
if nav=="match analysis":
    ...
    if analyze and date and first_innings and second_innings and venue:
        ...
        st.plotly_chart(fig)
elif nav=="win percentage calculator":
    disable=False
    with col1: first_innings=st.selectbox("Team batting first",options=df2["Bat First"].unique(),index=None)
    with col2: second_innings=st.selectbox("Team batting second",options=df2["Bat Second"].unique(),index=None)
    ...
    with col2: second_innings=st.selectbox("Team batting second",options=df2["Bat Second"].unique(),index=None)
    with col1: venue=st.selectbox("Venue",options=df2["Venue"].unique(),index=None)
    with col2: last_five=st.number_input("Runs in last 5 overs(30 balls)",min_value=0,value=None)
    with col1: runs=st.number_input("Runs Scored",min_value=0,value=None)
    if last_five and runs and last_five>runs:
        disable=True
        st.error("Runs scored in last five overs cannot be greater than total runs scored.")
    else: disable=False
    with col2: overs=st.text_input("Overs Completed",value=None,disabled=disable)
    if overs and float(overs)>20.0:
        disable=True
    ...
    if overs:
        ...
        balls+=int(overs)*6
        if balls<30:
            st.error("Prediction can only be done only after the completion of five overs.")
            disable=True
        else: disable=False
        wickets=10-wicket
        balls_left=120-balls
        ...
        record=[first_innings,second_innings,venue,balls_left,wickets,crr,last_five]
        user_data=pd.DataFrame([record],columns=['batting_team',"bowling_team","city",'balls_left','wickets_left','crr','last_five'])
    chk=(not disable) and first_innings and second_innings and venue and last_five and (runs!=None) and overs and wickets
    col1,col2,col3,col4,col5=st.columns([1,1,1,1,1])
    with col3: predict=st.button("Predict",disabled=(not chk))
    if predict:
        pred=round(pipe.predict(user_data)[0])
        st.subheader("Predicted Score:")
        st.header(f"{pred+runs}")